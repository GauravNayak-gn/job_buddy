...
class RedisClient:
    """
    Centralized Redis client wrapper.
    ...
    def is_rate_limited(key, limit=5, period=60):
        ...
        Returns True if rate limited (limit exceeded), False otherwise.
        """
        try:
            ...
            logger.error("Redis rate limit check failed for key: %s, error: %s", key, e, exc_info=True)
            return False
